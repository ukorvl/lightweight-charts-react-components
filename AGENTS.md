...
- Be aware of repository symlinks when editing docs or agent config. Update the canonical target once instead of editing through multiple aliases: `README.md -> ./lib/README.md`, `LICENSE -> ./lib/LICENSE`, `CLAUDE.md -> AGENTS.md`, and `.claude/skills` plus `.cursor/skills` both point to `../.agents/skills`.
...
- Keep the versions in README files across the repository aligned with `lib/package.json`. Update them in the same release change when the library version changes.
